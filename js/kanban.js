...
import { squaresHidden, toggleSquares } from './week.js';
...
function closeAllMenus() {
  ['viewMenu', 'boardMenu', 'rangeMenu', 'hideMenu'].forEach(id => {
...
function squaresItemHTML() {
  return squaresHidden()
    ? '<button type="button" data-sq-toggle="1">' + ICON_GRID + '<span>Показать квадраты недели</span></button>'
    : '<button type="button" data-sq-toggle="0">' + ICON_GRID_OFF + '<span>Скрыть квадраты недели</span></button>';
}
...
function bindSquaresItem(m) {
  const b = m.querySelector('[data-sq-toggle]');
  if (!b) return;
  b.onclick = e => {
    e.stopPropagation();
    closeAllMenus();
...
function openHideMenu(anchor) {
  ...
  m.innerHTML = hideItemHTML() + (mobile ? squaresItemHTML() : '');
  showMenu(m);
  placeMenu(m, anchor);
  bindHideItem(m);
  bindSquaresItem(m);
...
function openViewMenu(anchor) {
  ...
  m.innerHTML =
    '<button type="button" data-v="week" class="' + (!isOpen ? 'on' : '') + '"><span class="cm-ico">' + VIEW_ICON_WEEK + '</span><span>Running-list</span></button>' +
    '<button type="button" data-v="kanban" class="' + (isOpen ? 'on' : '') + '"><span class="cm-ico">' + VIEW_ICON_KANBAN + '</span><span>kanban-доска</span></button>';
  showMenu(m);
  placeMenu(m, anchor);
...
export async function toggleKanbanView() {
  ...
  if (!isOpen) {
    ...
    exitAnims = [];
    if (left) exitAnims.push(left.animate([{ transform: 'translateX(0)' }, { transform: 'translateX(' + (-off) + 'px)' }], EXIT));
    if (right) exitAnims.push(right.animate([{ transform: 'translateX(0)' }, { transform: 'translateX(' + (-off) + 'px)' }], Object.assign({ delay: 120 }, EXIT)));
    ...
    const left = visiblePane(grid, '.p-left') || visiblePane(grid, '.p-mobile');
    const right = visiblePane(grid, '.p-right');
    [left, right].forEach(el => { if (el && el.getAnimations) el.getAnimations().forEach(x => x.cancel()); });
    document.dispatchEvent(new CustomEvent('grid-rendered'));
    setTopbarKb(false);
